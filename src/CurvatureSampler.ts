...
export class CurvatureSampler {
  ...
  constructor(device: GPUDevice, scene: SDFScene, numPoints: number) {
    ...
    // Create scale factors output buffer (one f32 per point)
    const scaleFactorsBufferSize = numPoints * 4; // f32 per point
  ...
  private buildPipeline(): GPUComputePipeline {
    ...
    const shaderCode = `
      ...
      @group(0) @binding(1) var<storage, read_write> scaleFactors: ScaleFactors;
      ...
      fn computeMain(@builtin(global_invocation_id) globalId: vec3u) {
        ...
        // Sample normals at offset positions and measure variation
        ...
        // High variation (corner/edge) -> scale = 0.3
        let scaleFactor = 1.0 - smoothstep(0.0, 0.5, avgVariation);
        let finalScale = mix(0.01, 1.0, scaleFactor);

        scaleFactors.values[index] = finalScale;
      }
    `;
